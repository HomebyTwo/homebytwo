...
class RouteTestCase(TestCase):
    ...
    def test_route_view_success_owner(self):
        route = RouteFactory(owner=self.user)
    ...
    def test_route_view_success_not_owner(self):
        route = RouteFactory(owner=UserFactory())
        url = reverse("routes:route", args=[route.id])
        edit_url = reverse("routes:edit", args=[route.id])
    ...
    def test_route_view_success_no_start_place(self):
        route = RouteFactory(owner=UserFactory(), start_place=None)
        url = reverse("routes:route", args=[route.id])
        route_name = route.name
        end_place_name = route.end_place.name
    ...
    def test_route_view_success_no_end_place(self):
        route = RouteFactory(owner=UserFactory(), end_place=None)
        url = reverse("routes:route", args=[route.id])
        route_name = route.name
        start_place_name = route.start_place.name
...
class ActivityTestCase(TestCase):
    ...
    def load_strava_activity_from_json(self, file):
        ...
        activity_url = (
            "https://www.strava.com/api/v3/activities/%s" % strava_activity_id
        )
    ...
    def test_update_strava_activity_deleted(self):
        ...
        activity_url = (
            "https://www.strava.com/api/v3/activities/%s" % activity.strava_id
        )
    ...
    def test_import_all_user_activities_from_strava(self):
        ...
        responses = [
            ...
            httpretty.Response(
                content_type="application/json",
                body=read_data("activities.json"),  # two activities
            ),
            # second call, content one activity
            httpretty.Response(
                content_type="application/json",
                body=read_data("activities_one.json"),  # one activity
            ),
            httpretty.Response(
                content_type="application/json",
                body=read_data("activities.json"),  # two activities
